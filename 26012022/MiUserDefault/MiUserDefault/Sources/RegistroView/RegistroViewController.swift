...
class RegistroViewController: UIViewController {
    ...
    @IBAction func guardarDatosACTION(_ sender: Any) {
        ...
        if self.datosCumplimentados(){
            ...
            self.present(Utils.shared.muestraAlerta(titulo: "Saludos a todos", mensaje: "Estamos salvando los datos con exito", completionHandler: { (_) in
                self.usuarioLogado = true
                Utils.Constants().kPrefs.setValue(self.usuarioLogado, forKey: Utils.Constants().kUserLogeado)
            ...
            self.present(Utils.shared.muestraAlerta(titulo: "Debes cumplimentar el formulario", mensaje: "Adelante selecciona una foto y te registraras con exito", completionHandler: nil), animated: true, completion: nil)
        }
    }
    ...
    private func muestraPhotoMenu(){
        ...
        actionSheetVC.addAction(UIAlertAction(title: "Escoger de la libreeria", style: .default, handler: { _ in
            self.muestraPhotoLibrary()
        }))
...
extension RegistroViewController: UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [UIImagePickerController.InfoKey : Any]) {
        if let pickerImageUW = info[.originalImage] as? UIImage {
            self.imagenPerfil.contentMode = .scaleAspectFill
            self.imagenPerfil.image = pickerImageUW
