...
enum AuthenticationType: String {
    ...
    var footerText: String {
        ...
        case .singUp:
            return "Sing up for the R3D experience"
        }
    }
}
    ...
    private func handlerAuthenticationState(with auth: AuthDataResult?, and error: Error?){
        DispatchQueue.main.async {
            ...
            if let user = auth?.user{
                ...
                // TODO: meter mensaje de error de autentificacion
                CustomAlertView(title: "Error \(errorUnw)",
                                message: "El usuario o la contraseña no son correctos, si ya esta registrado, intentelo de nuevo",
                                hide: .constant(true)
                )
            }
        }
