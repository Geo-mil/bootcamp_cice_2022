...
protocol NetworkServiceProtocol {
    func requestGeneric<M: Decodable>(requestPayload: RequestDTO, entityClass: M.Type, success: @escaping(M?) -> Void, failure: @escaping(NetworkError) -> Void)
}
...
final class NetworkService: NetworkServiceProtocol{
    ...
    func requestGeneric<M>(requestPayload: RequestDTO,
        ...
        session.dataTask(with: urlRequest){ [weak self] (data, response, error) in
            ...
            guard let httpResponseUnw = response as? HTTPURLResponse, 200..<300 ~=
                    httpResponseUnw.statusCode else {
